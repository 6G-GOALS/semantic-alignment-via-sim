...
def mimo_equalization(
    channel: torch.tensor, snr_db: float = None
) -> torch.tensor:
    """A function that equalize the channel H and return the correspective equalizer G.
    If there is no noise then Zero Forcing is applyied, otherwise MMSE.

    Args:
        channel : torch.tensor
            The channel matrix H.
        snr_df : float
            The snr expressed in dB. Default None.

    Returns:
        G : torch.tensor
            The equalizer G.
    """
    antenna_receiver, _ = channel.shape

    if snr_db is None:
        # Perform equaization via Zero Forcing
        G = torch.linalg.inv(channel.H @ channel) @ channel.H
    else:
        # Perform equalization via MMSE
        ...
        G = (
            torch.linalg.inv(
                channel.H @ channel
                + (1 / snr_linear)
                * torch.eye(antenna_receiver, dtype=channel.dtype)
            )
            @ channel.H
        )
    return G
